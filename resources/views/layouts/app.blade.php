...
<head>
    ...
    <style>
        :root{--bg:#f4f5f7;--bg2:#fff;--bg3:#eaecef;--tx:#1a1a2e;--tx2:#555770;--tx3:#8b8da3;--pr:#4f46e5;--pr-h:#4338ca;--pr-l:#eef2ff;--gn:#16a34a;--gn-l:#dcfce7;--am:#d97706;--am-l:#fef3c7;--rd:#dc2626;--rd-l:#fee2e2;--bl:#2563eb;--bl-l:#dbeafe;--border:#d1d5db;--radius:8px;--font:'Outfit',sans-serif}
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        body{font-family:var(--font);background:var(--bg);color:var(--tx);font-size:14px;line-height:1.5}
        button{cursor:pointer;font-family:var(--font);font-size:13px;border:none;border-radius:var(--radius);padding:8px 16px;transition:background .15s}
        input,select,textarea{font-family:var(--font);font-size:13px;border:1px solid var(--border);border-radius:var(--radius);padding:8px 12px;width:100%;outline:none;background:var(--bg2)}
        ...
        th::after{content:'↕';position:absolute;right:5px;opacity:.2;font-size:14px;top:50%;transform:translateY(-50%)}
        ...
        .btn-pr:hover{background:var(--pr-h)}
        .btn-gn{background:var(--gn);color:#fff}
        .btn-gn:hover{opacity:.9}
        .btn-rd{background:var(--rd);color:#fff}
        .btn-rd:hover{opacity:.9}
        .btn-am{background:var(--am);color:#fff}
        .btn-am:hover{opacity:.9}
        .btn-o{background:transparent;border:1px solid var(--border);color:var(--tx2)}
        ...
        .chart-bar-wrap{display:flex;align-items:flex-end;gap:8px;height:180px;padding-top:20px}
        .chart-bar{display:flex;flex-direction:column;align-items:center;flex:1}
        .bar{background:var(--pr-l);border-radius:4px 4px 0 0;width:100%;transition:height .3s}
        .bar.today{background:var(--pr)}
        .bar-val{font-size:11px;color:var(--tx2);margin-bottom:4px}
        .bar-lbl{font-size:11px;color:var(--tx3);margin-top:6px}
...
<body dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
    <div class="mobile-header">
        ...
        <a href="{{ route('dashboard') }}" style="text-decoration:none;"><div class="logo-brand"><div class="logo-icon"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 9h18v10a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V9Z"/><path d="M3 9V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v4"/></svg></div><div class="logo-text"><span class="gen">Gen</span><span class="shelf">Shelf</span></div></div></a>
        <div style="width:24px;"></div>
    </div>
    <div class="overlay" id="mobile-overlay" onclick="toggleSidebar()"></div>
    <div class="app-container">
        <div class="sidebar" id="app-sidebar">
            <a href="{{ route('dashboard') }}" class="logo"><div class="logo-brand"><div class="logo-icon"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 9h18v10a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V9Z"/><path d="M3 9V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v4"/></svg></div><div class="logo-text"><span class="gen">Gen</span><span class="shelf">Shelf</span></div></div></a>
            <nav>
...
@if(in_array('offers', $roles))<a href="{{ route('offers') }}" class="{{ request()->routeIs('offers') ? 'active' : '' }}">⭐ {{ __('Special Offers') }}</a>@endif
@if(in_array('returns', $roles))<a href="{{ route('returns') }}" class="{{ request()->routeIs('returns') ? 'active' : '' }}">📦 {{ __('Returns') }}</a>@endif
@if(in_array('finance', $roles))<a href="{{ route('finance') }}" class="{{ request()->routeIs('finance') ? 'active' : '' }}">💰 {{ __('Finance') }}</a>@endif
@if(in_array('reports', $roles))<a href="{{ route('reports') }}" class="{{ request()->routeIs('reports') ? 'active' : '' }}">📈 {{ __('Reports') }}</a>@endif
...
@if(in_array('users', $roles))<a href="{{ route('users') }}" class="{{ request()->routeIs('users') ? 'active' : '' }}">🔑 {{ __('Users') }}</a>@endif
            ...
            <div class="sidebar-actions">
                <a href="{{ route('set-language', app()->getLocale() === 'ar' ? 'en' : 'ar') }}" class="lang-btn">{{ app()->getLocale() === 'ar' ? 'English' : 'العربية' }}</a>
...
@if(session('error'))<div style="background:var(--rd-l);color:var(--rd);padding:12px 16px;border-radius:var(--radius);margin-bottom:16px;font-weight:600;">{{ session('error') }}</div>@endif
            @yield('content')
