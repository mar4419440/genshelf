...
@push('scripts')
    <script>
        ...
        function openProductModal(product = null) {
            ...
            let p = isEdit ? product : { name: '', name_en: '', category: '', default_price: '', low_stock_threshold: '{{ $lowStockDefault }}', is_service: 0 };
            ...
            let categoryOptions = `<option value="">{{ __('Select Category') }}</option>` +
                categories.map(c => `<option value="${c.id}" ${p && p.category === c.name ? 'selected' : ''}>${c.name}${c.name_en ? ' - ' + c.name_en : ''}</option>`).join('');
