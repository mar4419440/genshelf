    ...
    <div class="card">
        ...
        <form action="{{ route('settings.updateToggles') }}" method="POST">
            ...
            @foreach($toggles as $key => $label)
                <label style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--bg3);font-size:13px">
                    <span>{{ $label }}</span>
            ...
            <button type="submit" class="btn btn-pr" style="margin-top:16px; width: 100%;">💾 {{ __('Save Toggles') }}</button>
        </form>
    </div>
</div>
@endsection
