...
<style>
/* ===== POS SPECIFIC ===== */
.pos-products { display: grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 10px; max-height: 65vh; overflow-y: auto; padding: 4px; }
.pos-card { background: var(--bg2); border: 1px solid var(--border); border-radius: var(--radius); padding: 14px; cursor: pointer; transition: border-color .15s; text-align: center; }
.pos-card:hover { border-color: var(--pr); }
.pos-card .pc-name { font-weight: 600; font-size: 13px; margin-bottom: 4px; }
.pos-card .pc-price { color: var(--pr); font-weight: 700; font-size: 15px; }
.pos-card .pc-stock { font-size: 11px; color: var(--tx3); margin-top: 4px; }

.cart-box { background: var(--bg2); border: 1px solid var(--border); border-radius: var(--radius); padding: 20px; }
.cart-items { max-height: 40vh; overflow-y: auto; }
.cart-item { display: flex; align-items: center; gap: 8px; padding: 8px 0; border-bottom: 1px solid var(--bg3); font-size: 13px; }
.cart-item .ci-name { flex: 1; font-weight: 500; }
.cart-item .ci-price-input { width: 70px; text-align: center; font-size: 12px; padding: 4px; }
.cart-item .ci-qty { display: flex; align-items: center; gap: 4px; }
.cart-item .ci-qty button { width: 24px; height: 24px; padding: 0; font-size: 14px; display: flex; align-items: center; justify-content: center; background: var(--bg3); border-radius: 4px; }
.cart-item .ci-qty span { width: 24px; text-align: center; font-weight: 600; }
.cart-item .ci-total { font-weight: 600; width: 70px; text-align: right; }
.cart-item .ci-del { background: none; color: var(--rd); font-size: 16px; padding: 2px 6px; }

.cart-summary { border-top: 2px solid var(--tx); padding-top: 12px; margin-top: 12px; }
.cart-summary .cs-row { display: flex; justify-content: space-between; padding: 3px 0; font-size: 13px; }
.cart-summary .cs-total { font-size: 18px; font-weight: 700; color: var(--pr); border-top: 1px solid var(--border); padding-top: 8px; margin-top: 4px; }
.cart-actions { display: flex; gap: 8px; margin-top: 14px; }
.cart-actions .btn { flex: 1; justify-content: center; padding: 10px; }
</style>
@endpush
    ...
    <div>
        ...
        <div class="pos-products" id="pos-product-grid">
            @forelse($products as $p)
                <div class="pos-card" onclick="addToCart('{{ $p->id }}', '{{ addslashes($p->name) }}', {{ $p->default_price }}, {{ $p->current_stock }})">
