...
@push('styles')
    <style>
        .pos-layout {
            ...
            .pos-item-card:hover { border-color: var(--pr); transform: translateY(-2px); }
            .pos-item-card:active { transform: scale(0.98); }
            .pic-name { font-weight: 700; font-size: 14px; margin-bottom: 4px; }
            .pic-bottom { display: flex; justify-content: space-between; align-items: flex-end; }
            .pic-price { color: var(--pr); font-weight: 800; font-size: 16px; }
            .pic-stock { font-size: 11px; color: var(--tx3); font-weight: 600; }
            ...
            .cir-top { display: flex; justify-content: space-between; align-items: flex-start; }
            .cir-name { font-weight: 600; font-size: 13px; flex: 1; }
            .cir-del { color: var(--rd); background: none; padding: 0 4px; font-size: 14px; }
            .cir-actions { display: flex; justify-content: space-between; align-items: center; }
            .cir-qty { display: flex; align-items: center; gap: 8px; }
            .qty-btn { width: 26px; height: 26px; border-radius: 6px; background: var(--bg3); display: flex; align-items: center; justify-content: center; font-size: 16px; }
            .qty-input { width: 45px; text-align: center; border: 1px solid var(--border); border-radius: 6px; background: var(--bg); font-weight: 700; }

            .pos-totals { border-top: 2px solid var(--border); padding-top: 16px; }
            .total-row { display: flex; justify-content: space-between; margin-bottom: 6px; font-size: 14px; }
            .grand-total { font-size: 24px; font-weight: 800; color: var(--pr); margin-top: 8px; border-top: 1px solid var(--bg3); padding-top: 8px; }

            .checkout-btn { width: 100%; padding: 16px; border-radius: 12px; font-size: 18px; font-weight: 800; text-transform: uppercase; margin-top: 16px; }
        </style>
@endpush
        ...
        <div class="pos-main">
            <div class="pos-header">
                <div style="display:flex; gap:12px; align-items:center;">
                    ...
                    <input type="text" id="pos-search" class="search-bar" placeholder="{{ __('Search products...') }}" style="width: 250px;">
                </div>
            ...
            <div class="pos-products-grid" id="pos-product-grid">
                @foreach($products as $p)
                    <div class="pos-item-card" data-cat="{{ strtolower($p->category) }}" onclick="addToCart('{{ $p->id }}', '{{ addslashes($p->name) }}', {{ $p->default_price }}, {{ $p->current_stock }})">
        ...
        <div class="pos-sidebar">
            ...
            <form action="{{ route('pos.checkout') }}" method="POST" id="checkout-form">
                ...
                <div style="position:relative; margin-bottom:12px;">
                    ...
                    <input type="text" id="customer-search" class="search-bar" style="width:100%" placeholder="{{ __('Find Customer...') }}" onfocus="showCustomerDropdown()" oninput="filterCustomerDropdown(this.value)">
                    <div id="customer-dropdown" class="card" style="position:absolute; width:100%; display:none; z-index:100; max-height:200px; overflow-y:auto; padding:0;">
                         <div style="padding:10px; cursor:pointer;" onclick="selectCustomer('', 'Walk-in')">{{ __('Walk-in Customer') }}</div>
                         @foreach($customers as $c)
                            <div class="customer-item" data-name="{{ strtolower($c->name) }}" onclick="selectCustomer('{{ $c->id }}', '{{ addslashes($c->name) }}')" style="padding:10px; cursor:pointer; border-top:1px solid var(--border);">{{ $c->name }}</div>
                         @endforeach
                    </div>
                </div>
                ...
                <div class="pos-totals">
                    ...
                    <div class="total-row"><span>{{ __('Tax') }} ({{ $taxRate }}%)</span> <span id="summary-tax">0.00</span></div>
                    ...
                    <div style="margin-top:12px; background:var(--bg3); padding:10px; border-radius:8px;">
                        ...
                        <input type="number" name="paid_amount" id="cart-paid-amount" step="0.01" class="search-bar" style="width:100%; border:none; background:none; padding:4px 0; font-size:16px; font-weight:700;" placeholder="Full Payment">
                    </div>
                    ...
                    <button type="button" class="btn btn-pr checkout-btn" onclick="submitCheckout()">{{ __('Place Order') }}</button>
                    <button type="button" class="btn btn-o btn-sm" style="width:100%; border:none;" onclick="clearCart()">{{ __('Clear Cart') }}</button>
                </div>
            </form>
        </div>
    </div>
