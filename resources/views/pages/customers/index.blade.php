...
<div class="card">
    <div class="table-wrap">
        <table>
                ...
                <tr>
                    ...
                    <td class="no-sort">
                        ...
                        @if($toggleCredit && $c->credit_balance > 0)
                            <button class="btn btn-xs btn-gn" onclick="recordPayment('{{ $c->id }}')">{{ __('Record Payment') }}</button>
                        @endif
                        <button class="btn btn-xs btn-bl" onclick="viewCustomerHistory('{{ $c->id }}')">{{ __('History') }}</button>
                        
                        <form action="{{ route('customers.destroy', $c->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('{{ __('Are you sure?') }}')">
                            @csrf @method('DELETE')
            ...
            @empty
                <tr><td colspan="{{ 4 + ($toggleCredit?1:0) + ($toggleLoyalty?1:0) }}" class="empty-state">{{ __('No data yet') }}</td></tr>
            @endforelse
        </table>
    </div>
</div>
