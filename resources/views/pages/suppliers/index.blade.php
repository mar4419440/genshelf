        ...
        <div class="card">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
                ...
                <div style="display:flex;gap:6px">
                    <a href="{{ route('suppliers.template') }}" class="btn btn-sm btn-o">📥 {{ __('Template') }}</a>
                    <button class="btn btn-sm btn-o" onclick="document.getElementById('import-supplier-file').click()">📤 {{ __('Import File') }}</button>
                    <button class="btn btn-sm btn-pr" onclick="openSupplierModal()">➕ {{ __('Add Supplier') }}</button>
                    
                    <form id="import-supplier-form" action="{{ route('suppliers.import') }}" method="POST" enctype="multipart/form-data" style="display:none;">
                        @csrf
                        <input type="file" id="import-supplier-file" name="csv_file" onchange="document.getElementById('import-supplier-form').submit()">
                    </form>
                </div>
            </div>
