            ...
            <table>
                ...
                @forelse($suppliers as $s)
                    <tr>
                        ...
                        <td>{{ $s->phone }}</td>
                        <td class="no-sort">
                            <button class="btn btn-xs btn-o" onclick='openSupplierModal(@json($s))'>{{ __('Edit') }}</button>
                            <form action="{{ route('suppliers.destroy', $s->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('{{ __('Are you sure?') }}')">
                                @csrf @method('DELETE')
    ...
    <div class="card">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
            ...
            <div style="display:flex;gap:8px">
                ...
                <button class="btn btn-sm btn-o" onclick="document.getElementById('import-po-file').click()">📤 {{ __('Import PO CSV') }}</button>
                <button class="btn btn-sm btn-pr" onclick="openPOModal()">➕ {{ __('New PO') }}</button>
                
                <form id="import-po-form" action="{{ route('suppliers.po.import') }}" method="POST" enctype="multipart/form-data" style="display:none;">
                    @csrf
                    <input type="file" id="import-po-file" name="csv_file" onchange="document.getElementById('import-po-form').submit()">
                </form>
            </div>
        </div>
        <div class="table-wrap">
                    ...
                    <tr>
                        ...
                        <td class="no-sort">
                            @if($po->status === 'pending')
                                <form action="{{ route('purchase-orders.receive', $po->id) }}" method="POST" style="display:inline;">
                                    @csrf
